...
object Mythic4Listener : Listener {
    @EventHandler
    fun onRegisterDrop(event: MythicDropLoadEvent) {
        if (event.dropName.equals("PlayerExp", ignoreCase = true)) {
            event.register(Mythic4ExpDrop(event.container.configLine, event.config))
        }
    }
}

class Mythic4ExpDrop(line: String, config: MythicLineConfig) : Drop(line, config), IItemDrop {
