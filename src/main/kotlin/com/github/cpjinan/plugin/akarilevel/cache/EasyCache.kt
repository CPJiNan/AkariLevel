...
class EasyCache<K : Any, V : Any> private constructor(
    ...
    fun compute(key: K, remappingFunction: (K, V?) -> V?): V? {
        ...
        return try {
            val currentValue = if (loadingCache != null) loadingCache.get(key)
            else caffeineCache.getIfPresent(key)

            val newValue = remappingFunction(key, currentValue)

            if (newValue != null) caffeineCache.put(key, newValue)
            else if (currentValue != null) caffeineCache.invalidate(key)
